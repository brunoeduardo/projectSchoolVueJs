<template>
  <div class="app-container">
    <Title textTitle="Student"/>
    <input type="text" class="input-name" placeholder="Student name" v-model="name" v-on:keyup.enter="addStudent">
    <button type="submit" class="input-submit" v-on:click="addStudent">ADD</button>
    <table class="students-table">
      ...
      <tbody class="students-table-body" v-if="students.length">
        <tr v-for="(student, index) in students" :key="index">
          <td>{{student.id}}</td>
          <td class="students-table-col-name">{{student.name}} {{student.surname}}</td>
          <td>
...
export default {
  ...
  methods: {
    addStudent() {
      ...
      this.$http.post('http://localhost:3000/students', _student)
        .then(result => result.json())
    ...
    removeStudent(student) {
        this.$http.delete(`http://localhost:3000/students/${student.id}`)
          .then(() => {
